	...
	)
{
	...
	if(plotit==TRUE){plot(phy)}
		
  dn = round(dist.nodes(phy),4) # find distances between nodes
  ...
  patable = data.frame(p=phy$edge[,1],d=phy$edge[,2],age.start=rep(NA,nbranch),age.end=rep(NA,nbranch),extant=rep(0,nbranch),state=rep(NA,nbranch))
	...
	if(nTdim>1){ # store traits as a vector or a matrix for 1 or multiple dimensions
	...
	while(tstep < max(patable$age.end)){
		...
		if(tstep < max(patable$age.end)){ # if we have yet to reach the present then simulate the next event 
			if(tstep < min(patable$age.end[extant])){ # if the next event is an immigration or local extinction event 
				
				Itot.real = 0
				
				if(length(pa)-sum(pa)>0){ # if there are species absent from the community
					
					Dpaynotpa = 1 - pa
					Itot.real = gamma_0 * sum(Dpaynotpa)
					
					if(gamma_td>0){ # if we have a slowdown in immigration rates.......
						Dpaynotpa = 1 - pa
						Dpaynotpa = gamma_0/(1+(gamma_td*tstep))*Dpaynotpa
						Itot.real = sum(Dpaynotpa)
					}
					
					if(psiBranch>0 & sum(pa)>0){ #	if we have phylogenetic repulsion and species are present in the community already...........
            ...
            D[which(pa == 0),] = NaN
					...
					if(phi>0){ # if we have environmental filtering.......
						if(nTdim>1){ # calculate distance of species from trait optima
					...
					if(psiTrait>0 & sum(pa)>0){  # if we have trait repulsion.......
				...
				if(event == 1){ # if its an immigration event.....
					if(sum(1 - pa) == 1){
						patable$state[extant[which(pa == 0)]] = 1	
					}else{	
						patable$state[sample(extant, 1, replace = FALSE, prob = Dpaynotpa)] = 1
					}
					if(plotit == TRUE){lines(c(tstep,tstep),c(0,100),col = "lightblue",lty=2)}	
				} 
				if(event == 2){	# if its an local extinction event........
					if(sum(pa) == 1){patable$state[extant[which(pa == 1)]] = 0 }else{ patable$state[sample(extant[which(pa ==1 )], 1)] = 0}
					if(plotit == TRUE){lines(c(tstep,tstep),c(0,100),col="orange",lty=2)}
				} 			
				...
				for(fe in unique(focedge)){ # there may be multiple events at the same time  - so loop through these
					dedge = which(patable$p == patable$d[fe])
					if(length(dedge)>0){	# if its a speciation event......
	...
	if(keepExtinct == FALSE){
		if(nTdim>1){ # prune out extinct species
